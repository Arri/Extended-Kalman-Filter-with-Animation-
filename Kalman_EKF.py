...
class System:
    ...
    def getAccelVector(self, a):
        """ 
            This function gets the 3D values (accX, accY, accZ) from the accelerator
            and normalizes the values over the magnitude of the 3 dimensions
        """
        accel = np.array(a).transpose()
        accelMag = (accel[0] ** 2 + accel[1] ** 2 + accel[2] ** 2) ** 0.5
        # print(f"Acceleroneter = {accel}")
    ...
    def getMagVector(self, m):
        ...
        magGauss_B = np.matmul(getRotMat(self.xHat).transpose(), magGauss_N)
        # print(f"Magnetometer = {magGauss_B}")
        return magGauss_B
